...
const MappedNotifications = props => {
  ...
  const getFollowing = async () => {
    ...
    if (props.thisNotification.is_follow) {
      ...
      await axios
        .get(`/api/getspecificfollowing/${props.userReducer.user.id}`)
        .then(res => setFollowing(res.data))
  ...
  const follow = () => {
    axios
      .post("/api/follow", { followed_id: props.thisNotification.id })
      .then(res => {
        setFollowed(true);
      });
    followNotification();
  ...
  const followNotification = () => {
    ...
    axios
      .post("/api/addnotification/", {
        receiver_id: props.thisNotification.id,
        post_id: 1,
        is_comment: false,
        is_like: false,
        is_follow: true,
    ...
    <>
      {props.thisNotification.is_like ? (
        <div className="notificationContainer">
          <img
            className="senderProfPic"
            src={
              props.thisNotification.profile_img
                ? props.thisNotification.profile_img
                : `https://robohash.org/${props.thisNotification.username}?set=set5`
            }
            alt="profpic"
          />
        ...
        <div className="notificationContainer">
          ...
          <div className="message">
            <strong>{props.thisNotification.username}</strong> commented:{" "}
            {props.thisNotification.contents}
          </div>
        ...
        <div className="notificationContainer">
          ...
          <div className="message">
            <strong>{props.thisNotification.username}</strong> started following
            you.
          </div>
